...
(async () => {
    fs.writeFileSync('./Monty_Hall_Problem/logs/log.txt', "");
    console.log(`Starting the simulation with ${attempts} rounds in 2 seconds...`);
    fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `Starting the simulation with ${attempts} rounds in 2 seconds with logging...\n`); //Logs
    await Sleep(1000);
    console.log(`Starting the simulation with ${attempts} rounds in 1 seconds...`);
    fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `Starting the simulation with ${attempts} rounds in 1 seconds with logging...\n`); //Logs
    await Sleep(1000);
    console.log(`Starting the simulation with ${attempts} rounds in 0 seconds...`);
    fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `Starting the simulation with ${attempts} rounds in 0 seconds with logging...\n`); //Logs

    let wins = 0;
    let fails = 0;
    for(let i = 0; i < attempts; i++) {  //Amount of times to run the simulation.
        console.log("Starting new game...");
        fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `Starting new game...\n`); //Logs
        //Get the treasure's door
        let treasureDoor = Math.floor(Math.random() * 3 + 1);
        console.log(`  ├─treasureDoor: ${treasureDoor} <-`);
        fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `  ├─treasureDoor: ${treasureDoor} <-\n`); //Logs
        Sleep(delay);

        //Get the user's door choice
        let userDoor = Math.floor(Math.random() * 3 + 1);
        console.log(`  ├─userDoor: ${userDoor} <-`);
        fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `  ├─userDoor: ${userDoor} <-\n`); //Logs
        Sleep(delay);
        ...
        async function GethostDoor() {
            ...
            while(hostDoor == treasureDoor || hostDoor == userDoor) {
                ...
                fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `  ├─debug: trying hostDoor: ${hostDoor}\n`); //Logs
            }
            return hostDoor;
        }
        let hostDoor = await GethostDoor()
        console.log(`  └─hostDoor: ${hostDoor} <-`);
        fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `  └─hostDoor: ${hostDoor} <-\n`); //Logs
        await Sleep(delay);

        //Switching the user's door
        console.log("Switching the user's door...");
        fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `Switching the user's door...\n`); //Logs
        async function GetNewUserDoor(userDoor, hostDoor) {
            ...
            while(newUserDoor == userDoor || newUserDoor == hostDoor) {
                ...
                fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `  ├─debug: trying door: ${newUserDoor}\n`); //Logs
            }
        ...
        fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `  └─newUserDoor: ${newUserDoor}\n`); //Logs
        await Sleep(delay);
        ...
        console.log("------------------------------")
        fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `------------------------------\n`); //Logs
        console.log(`Treasure door: ${treasureDoor}`);
        fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `Treasure door: ${treasureDoor}\n`); //Logs
        console.log(`User door: ${userDoor}`);
        fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `User door: ${userDoor}\n`); //Logs
        console.log(`Game ${i + 1} (${i}) Result: ${result}`);
        fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `Game ${i + 1} (${i}) Result: ${result}\n`); //Logs
        console.log("------------------------------");
        fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `------------------------------\n`); //Logs
        console.log(`========================================================`);
        fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `========================================================\n`); //Logs
    ...
    fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `Simulation results:\n`); //Logs
    console.log(`Attempts: ${wins + fails}`);
    fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `Attempts: ${wins + fails}\n`); //Logs
    console.log(`Wins: ${wins}`);
    fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `Wins: ${wins}\n`); //Logs
    console.log(`Fails: ${fails}`);
    fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `Fails: ${fails}\n`); //Logs
    console.log(`Theoretical win probability: ${(wins + fails) / 3 * 2}/${wins + fails} (${(((wins + fails) / 3 * 2) / (wins + fails)) * 100}%)`);
    fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `Theoretical win probability: ${(wins + fails) / 3}/${wins + fails} (${attempts * (1 / 3)}%)\n`); //Logs
    console.log(`Experimental win probability: ${wins}/${wins + fails} (${(wins / (wins + fails)) * 100}%)`);
    fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `Experimental win probability: ${wins}/${wins + fails}(${(wins / (wins + fails)) * 100}%)\n`); //Logs
    console.log("------------------------------")
    fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `------------------------------\n`); //Logs
    console.log("Code written by Jerry.");
    fs.appendFileSync('./Monty_Hall_Problem/logs/log.txt', `Code written by Jerry.\n`); //Logs
})();
